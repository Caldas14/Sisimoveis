...
interface ExcelExporterExcelJSProps {
  imovel: Imovel | null;
...
const ExcelExporterExcelJS: React.FC<ExcelExporterExcelJSProps> = ({ 
  ...
  const gerarExcelImediato = async () => {
    ...
    // Logs para debug dos valores das matrículas originadas
    console.log(`Exportando imóvel principal: ${imovel.matricula}`);
    console.log(`Matrículas originadas do principal: ${imovel.matriculasOriginadas || 'N/A'}`);
    
    if (imoveisSecundarios && imoveisSecundarios.length > 0) {
      console.log(`${imoveisSecundarios.length} imóveis secundários para exportar`);
      imoveisSecundarios.forEach((sec, i) => {
    ...
    // Criar uma planilha para o imóvel
    const worksheet = workbook.addWorksheet('Imóvel');
    ...
    saveAs(blob, `imovel_${imovel.matricula}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  return (
    <button 
      onClick={gerarExcelImediato}
      className={`px-4 py-2 border rounded-lg shadow-sm transition-colors duration-200 flex items-center ${
        darkMode 
